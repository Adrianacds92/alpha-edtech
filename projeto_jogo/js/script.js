class Hero {
    constructor(name, type, universe, st, agi, int, _src, _hero, _music, _gif, _alt, _typeFight) {
        ...
        this.hp = this.st * 5 //Pontos de Vida
        this.hpMax = this.hp
...
$('#btn-end').on({
    click: function () {
        ...
        $('.fight-dc').html("");
        $('#btn-fight').css('visibility', 'hidden')
        console.log(playerDC)
        console.log(playerMarvel);
    }
})
...
function grayscaleBtn(_element) {
    return element.css('filter', 'grayscale(100%)')
}
...
function attackHero(_playerAttck, _playerDefender) {
    ...
    if (randomRange(40, 240) < playerDefender.def && typeAttack === "st") {
        console.log("defesa", damage)
        return damage;
    }
    if (randomRange(120, 800) < playerAttck.mp) {
        console.log("dano critico", playerAttck[typeAttack] * 2)
        return dange = playerAttck[typeAttack] * 2
    }
    console.log("normal", playerAttck[typeAttack])
